...
def pure_literals(formula):
    ...
    for pure in pures:
        formula = reduce_formula(formula, pure)
    assignment+=pures
...
def unit_propagation(formula):
    ...
    while len(unit_clauses) > 0:
        ...
        if formula == -1:
            return -1, []
        print("after unit propagation formula reduced to",end=" ")
...
def backtracking(formula, assignment):
    print("formula is",end=" ")
    print_formula(formula)
    formula,pure_assignment=pure_literals(formula)
    print("after pure literal reduction formula is",end=" ")
    print_formula(formula)
    formula, unit_assignment = unit_propagation(formula)
   
    assignment = assignment +  unit_assignment+pure_assignment
    ## formula contains an empty clause, formula is unsatisfiable
    if formula == - 1:
        return []
    ## formula is empty, formula is satisfiable
    if formula==[]:
        return assignment
